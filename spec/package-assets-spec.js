...
describe("inlay-hints package assets", () => {
  ...
  it("ships a CSS stylesheet built on custom properties, not Less", () => {
    ...
    expect(exists("styles/inlay-hints.less")).toBe(false);
  ...
  it("is named `inlay-hints` and carries the lumine-code metadata", () => {
    const pkg = JSON.parse(read("package.json"));
    expect(pkg.name).toBe("inlay-hints");
    expect(pkg.author).toBe("lumine-code");
    expect(pkg.repository).toBe("https://github.com/lumine-code/inlay-hints");
    expect(pkg.bugs.url).toBe("https://github.com/lumine-code/inlay-hints/issues");
  ...
  it("keeps a keyword list that never repeats the package name", () => {
    ...
    for (const keyword of keywords) {
      ...
      expect("inlay-hints".includes(keyword)).toBe(false);
    }
  });

  it("defines a flat config schema under the inlay-hints namespace without order keys", () => {
    const schema = JSON.parse(read("package.json")).configSchema;
    expect(Object.keys(schema)).toEqual(["enabled", "maxLabelLength"]);
  ...
  it("places its commands in one Packages submenu named for the package", () => {
    ...
    const submenu = packages.submenu.find((item) => item.label === "Inlay Hints");
  ...
  it("ships the contract document for the service it owns", () => {
    expect(exists("docs/inlay-hints.provider.md")).toBe(true);
    const doc = read("docs/inlay-hints.provider.md");
    expect(doc.split(/\r?\n/)[0]).toBe("# inlay-hints.provider");
    expect(doc).toContain("provideInlayHints");
    expect(doc).toContain("consumeInlayHints");
  });

  it("keeps the README description in sync with package.json", () => {
    const pkg = JSON.parse(read("package.json"));
    const lines = read("README.md").split(/\r?\n/);
    expect(lines[0]).toBe("# inlay-hints");
